...
test('input fields',async({page})=>{
    ...
    // locator assertion
    await expect(usingTheGridEmailInput).toHaveValue('test2@test.com') // for input fields needs to be to have value
})
...
test('checkboxes',async({page})=>{
    ...
    for(const box of await allBoxes.all()){
        ...
        expect(await box.isChecked()).toBeTruthy() // to be falsy
    }
})  
...
test('List & Dropdowns',async({page})=>{
    ...
    page.getByRole('list')  // used when the list has UL tag
...
test('sliders',async({page})=>{
   ...
   // starting coordinates in the centar of the box
   const x= box.x + box.width/2
   const y= box.y + box.height/2
    await page.mouse.move(x,y)
    await page.mouse.down()
    await page.mouse.move(x+100,y) // move(x-100,y)
    await page.mouse.move(x+100,y+100) // move(x-100,y+100)
    await page.mouse.up()
